...
- **DataForSEO `orphan_pages` computation (pending verification)**: Currently counted from per-page `checks.is_orphan_page` in `dataforseo_crawl.py`. DataForSEO docs define this as "pages with no inbound internal links." The summary `page_metrics.checks` dict may expose `is_orphan_page` as an aggregate — if so, use `_summary_check(summary_result, "is_orphan_page", 0)` instead (simpler, consistent with all other fields). Verify on a real crawl by logging both values before switching.
